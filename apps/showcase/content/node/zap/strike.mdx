...
```typescript
const strikeRequest = s.make("/users", z.object({ id: z.string(), name: z.string() }));
```
...
const getItem = await strikeRequest.get(userId);
...
const main = async () => {
  const getted = await strikeRequest.get("1");
  console.log("Getted", getted);

  const posted = await strikeRequest.new({ title: "test", completed: false, userId: 1 });
  console.log("Posted", posted);

  const listed = await strikeRequest.list({ limit: 2, offset: 3 });
  console.log("Listed", listed);

  const updated = await strikeRequest.update("1", { title: "test2" });
